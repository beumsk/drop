...
function MAIN() {
  ...
  if (!startBtn || !restartBtn || !resumeBtn || !btnLeft || !btnRight) {
  ...
  class Entity implements EntityType {
    ...
    constructor(type: EntityType["type"]) {
      this.type = type;
  ...
  function bindTouchButton(element: HTMLElement, keyName: "ArrowLeft" | "ArrowRight") {
    const pressOn = (e: any) => {
      e.preventDefault();
      keys[keyName] = true;
    };
  ...
  function draw() {
    if (!ctx) return console.log("missing ctx");
